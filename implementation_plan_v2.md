...
> **Status:** 🚧 In Progress
> **Goal:** Transition the Unified System to a Distributed, Non-Federated Agent Architecture using Billboard, Mailboxes, Beads, and REP.
...
3.  **REP (Alignment Signal):** Change sensitivities ("IF X THEN Y") committed to the repo.
4.  **CI/CD (Sensor/Gate):** "Done" is derived only from merged code + green checks.
...
- [ ] **Directory Structure:** Create `billboard/`, `contexts/`, `rep/`.
- [ ] **Domain Vision:** Create `billboard/spec.md` identifying the Core Domain and Ubiquitous Language.
- [ ] **Task Template:** Create `billboard/tasks/TEMPLATE.md` with required frontmatter (`Task-Id`, `Context`, `Outcome`).
...
- [ ] **Context Mapping:** Identify Bounded Contexts for current scripts (e.g., `FamilyAssistant`, `ContentFactory`, `Infrastructure`).
- [ ] **Refactor:** Move `Scripts/` into `contexts/<ContextName>/`.
- [ ] **Backfill:** Create Billboard tasks for active work (e.g., "Fix YouTube Uploads").
...
*   **Semantic Slug:** `Context.Capability.Outcome` (e.g., `ContentFactory.Youtube.UploadVideo`)
