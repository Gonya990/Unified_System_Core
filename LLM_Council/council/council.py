...
class LLMCouncil:
    ...
    def from_token_broker(cls, broker, tier: Optional[str] = None) -> "LLMCouncil":
        """Create council using keys from TokenBroker. NVIDIA is prioritized."""
        providers = []

        # 1. NVIDIA (Primary & Chairman)
        nvidia_key = broker.get_key("nvidia", tier=tier)
        if nvidia_key:
            providers.append(NVIDIANIMProvider(api_key=nvidia_key, model="meta/llama-3.1-70b-instruct"))
            logger.info("✓ NVIDIA NIM provider initialized (Chairman)")

        # 2. Gemini (Disabled temporarily due to SDK 404 issues)
        # gemini_key = broker.get_key("gemini", tier=tier)
        # if gemini_key:
        #     providers.append(GeminiProvider(api_key=gemini_key, model="models/gemini-1.5-flash"))
        #     logger.info("✓ Gemini provider initialized")

        # 3. OpenAI (Disabled temporarily due to Quota issues)
        # openai_key = broker.get_key("openai", tier=tier)
        # if openai_key:
        #     providers.append(OpenAIProvider(api_key=openai_key, model="gpt-4o", store=False))
        #     logger.info("✓ OpenAI provider initialized")

        if not providers:
            raise ValueError("No valid API keys available from TokenBroker.")

        return cls(
            providers=providers,
            chairman=providers[0],  # NVIDIA if available
            enable_peer_review=len(providers) > 1,
        )
