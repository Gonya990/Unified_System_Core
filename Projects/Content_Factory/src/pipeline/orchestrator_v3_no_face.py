    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "-af",
                (
                    "bass=g=4:f=100,treble=g=2:f=8000,"
...
def transcribe_audio_gemini(audio_path: Path) -> list[dict]:
    ...
    try:
        ...
        prompt = (
            "Transcribe this audio. Return ONLY a JSON list of words with start "
            'and end timestamps in seconds: [{"word": "text", "start": 0.0, '
            '"end": 0.5}, ...]. Do not include markdown formatting, '
            "just raw JSON."
        )
...
def transcribe_audio_whisper(audio_path: Path) -> list[dict]:
    ...
    if api_key:
        ...
        try:
            subprocess.run(
                ["ffmpeg", "-y", "-i", str(audio_path), "-b:a", "128k", str(temp_mp3)],
                check=True,
                capture_output=True,
            )
...
def add_subtitles(
    ...
    subprocess.run(
        ["ffmpeg", "-y", "-i", str(video_path), "-vn", "-ac", "1", str(temp_audio)],
        check=True,
        capture_output=True,
    )
    ...
    for _i, w in enumerate(words):
        ...
        if style == "impact":
            ...
            f = (
                ...
                f"fontcolor=0xFFD700:fontsize=80:"
            ...
            f = (
                f"drawtext=fontfile='{font_path}':text='{text}':"
                f"fontcolor={color}:fontsize=100:"
...
def assemble_broll_only_video(audio_path: Path, clips: list[Path], output_path: Path):
    ...
    if not audio_duration or audio_duration <= 0:
        ...
        msg = (
            f"❌ Could not estimate segment duration: {audio_path.name}. "
            "Using fallback."
        )
        print(msg)
...
def assemble_hybrid_video(
    ...
    print(
        f"⏱️ Exact Scene Duration: {duration_per_scene:.2f}s "
        f"(Total: {audio_duration:.2f}s)"
    )

    segments = []

    # Generate unique prefix based on output filename to prevent
    ...
    for i, scene in enumerate(scenes):
        ...
        if broll_clips:
            # We need to cover broll_dur. If clips are too short,
            # we loop or use multiple.
            accumulated_broll = 0
            clip_idx = 0
            while accumulated_broll < broll_dur:
                ...
                if style == "impact":
                    vf = (
                        "scale=1080:1920:force_original_aspect_ratio=increase,"
                        "crop=1080:1920,eq=contrast=1.1:saturation=1.3,"
                        "vignette=angle=0.3"
                    )
                else:
                    vf = (
                        "scale=1080:1920:force_original_aspect_ratio=increase,"
                        "crop=1080:1920,eq=saturation=1.5"
                    )
        ...
        if style == "impact":
            vf_img = (
                ...
                "crop=1080:1920,fade=in:0:5:color=white,vignette=angle=0.5"
            )
        else:
            vf_img = (
                "scale=w=1080:h=1920:force_original_aspect_ratio=increase,"
                "crop=1080:1920,fade=in:0:5:color=pink"
            )
    ...
    try:
        ...
        print(f"❌ FFmpeg Merge Error: {e.stderr.decode() if e.stderr else e}")
        raise
...
if __name__ == "__main__":
    ...
    script_ru = (
        "Добро пожаловать в две тысячи двадцать шестой год. Мир, где "
        "искусственный интеллект стал частью нашей реальности... "
        "навсегда изменив экономику и общество. "
        "Пятнадцать триллионов долларов. Именно столько искусственный интеллект "
        "привнес в мировую экономику... став главным двигателем прогресса. "
        "Девяносто процентов контента, который мы видим каждый день... "
        "теперь создается алгоритмами. От новостей... до кино и музыки. "
        "Задачи, которые раньше требовали десятилетий... сегодня решаются за "
        "считанные недели. Это открывает перед человечеством новые... "
        "невероятные горизонты. "
        "Мы наблюдаем настоящую эволюцию разума... уникальный симбиоз "
        "человеческого опыта и цифрового интеллекта. "
        "Код становится новой материей... фундаментом, на котором строятся "
        "наши будущие творения. "
        "Автоматизация труда. Она не заменяет нас... она освобождает человека "
        "для истинного творчества и самовыражения. "
        "Искусственный интеллект вдохновляет на новые формы искусства... "
        "расширяя границы нашей фантазии до бесконечности. "
        "Образование трансформируется прямо сейчас... оно становится персональным... "
        "доступным каждому, в любой точке планеты. "
        "В медицине происходит революция. Цифровой разум обеспечивает каждому "
        "из нас доступ к диагностике... и лечению высочайшего уровня. "
        "Умные города и экологичные технологии. С помощью современных алгоритмов... "
        "мы находим способы сохранить наш общий дом... нашу планету. "
        "Но с великой силой приходят и вопросы... вопросы безопасности... "
        "и этики. Они требуют нашего глубокого внимания. "
        "Мы учимся взаимодействовать с новым типом разума... выстраивая мосты "
        "взаимопонимания... и сотрудничества. "
        "Глобальная сеть интеллектуальных систем объединяет мир... и ускоряет наше "
        "общее развитие... с каждым днем. "
        "Две тысячи двадцать шестой год. Это лишь начало новой эры... где разум и "
        "технологии создают совершенно новый мир... мир будущего."
    )

    # Mapping scenes to images (using the ones generated across steps)
    gen_dir = Path(
        "/Users/macbook/.gemini/antigravity/brain/"
        "74acf072-6bc0-4fdc-9ad0-33f04fb9fa16"
    )
    ...
    scene_data = [
        {
            "image": "ai_scene_1_network",
            "keyword": "cinematic nebula space 4k slow motion",
        },
        {"image": "ai_scene_2_economy", "keyword": "stormy ocean waves cinematic 4k"},
        {
            "image": "ai_scene_3_content_gen",
            "keyword": "lion roar slow motion cinematic",
        },
        ...
        {
            "image": "ai_scene_6_code_matter",
            "keyword": "matrix code rain slow motion cinematic",
        },
        {
            "image": "ai_future_bg_1",
            "keyword": "mountain fireplace cozy dark cinematic",
        },
        {
            "image": "ai_scene_8_art_exhibit",
            "keyword": "ink in water slow motion cinematic",
        },
        ...
        {
            "image": "ai_scene_11_green_city",
            "keyword": "waterfall cinematic 4k slow motion",
        },
        {
            "image": "ai_scene_12_ethics_safety",
            "keyword": "cyber attack digital shield cinematic",
        },
        {
            "image": "ai_fact_1_robot",
            "keyword": "artificial intelligence robot thinking cinematic",
        },
        {"image": "ai_scene_14_global_net_earth", "keyword": "planet earth from space"},
        {
            "image": "ai_scene_15_sunrise_digital",
            "keyword": "futuristic city sunrise cinematic",
        },
    ]
    ...
    if len(selected_scenes) >= 15:
        ...
        script_en = (
            "Welcome to the year twenty twenty-six... a world where artificial "
            "intelligence has become an inseparable part of our reality... "
            "forever changing our economy and our society. "
            "Fifteen trillion dollars. That is how much AI has contributed to the "
            "global wealth... becoming the ultimate engine of progress. "
            "Ninety percent of the content we consume every single day... is now "
            "generated by algorithms. From breaking news... to cinema and music. "
            "Tasks that used to take decades of research... are now solved in "
            "just a few weeks. This opens up new... incredible horizons for all "
            "of humanity. "
            "We are witnessing a true evolution of the mind... a unique symbiosis "
            "between human experience and digital intelligence. "
            "Code is becoming the new matter... the very foundation upon which our "
            "future creations are built. "
            "The automation of labor. It is not here to replace us... it is here "
            "to free us for true creativity and self-expression. "
            "Artificial intelligence inspires new forms of art... pushing the "
            "boundaries of our imagination to infinity. "
            "Education is transforming right before our eyes. It is becoming "
            "personal... accessible to everyone, anywhere on the planet. "
            "A revolution is happening in healthcare. AI provides each of us with "
            "diagnostics... and medical care of the highest standard. "
            "Smart cities and sustainable technologies. With the help of AI... we "
            "are finding ways to preserve our common home... our planet. "
            "But with great power come deep questions... questions of safety... "
            "and ethics. They demand our absolute attention. "
            "We are learning to interact with a new kind of consciousness... "
            "building bridges of understanding... and cooperation. "
            "A global network of intelligent systems is uniting the world... "
            "accelerating our collective progress... every single day. "
            "Twenty twenty-six. This is just the beginning of a new era... where "
        ...
        print(
            f"❌ Not enough images found ({len(selected_scenes)}/15). "
            f"Check gen_dir: {gen_dir}"
        )
