    ...
    for i, scene in enumerate(scenes):
        ...
        if not success:
            ...
            cmd = [
                ...
                "-vf", f"drawtext=text='{keyword}':fontcolor=white:fontsize=64:x=(w-text_w)/2:y=(h-text_h)/2",
                "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30",
                str(clip_raw)
            ]
            subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        ...
        cmd_norm = [
            "ffmpeg", "-y", "-i", str(clip_raw),
            "-t", str(scene_duration),
            "-vf", "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920",
            "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30", "-an", # No audio in segments
            str(clip_final)
        ]
...
if __name__ == "__main__":
    ...
    run_no_face_pipeline("This is a test of the fully operational video system.", lang="en", output_name="test_system_v3", scenes=scenes)
