            ...
            if category == 'text':
                ...
                if summary:
                    with sqlite3.connect(DB_PATH) as conn:
                        ...
                        # Refresh FTS
                        conn.execute('INSERT OR REPLACE INTO assets_fts(rowid, path, filename, concept_summary) SELECT id, path, filename, concept_summary FROM assets WHERE id = ?', (asset_id,))
                    return True
