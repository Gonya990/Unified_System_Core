...
class FirestoreDB:
    ...
    def update_last_interaction(self, user_id: int):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).update({"last_interaction": firestore.SERVER_TIMESTAMP})
        else:
    ...
    def approve_user(self, user_id: int, approve: bool = True):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).update({"is_approved": approve})
        else:
    ...
    def set_google_connected(self, user_id: int, connected: bool = True):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).update({"is_google_connected": connected})
        else:
    ...
    def add_memory(self, user_id: int, fact_short: str, fact_full: str):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).collection("memories").add(
                {
    ...
    def add_event_context(self, user_id: int, event_title: str, context_description: str, event_time: datetime):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).collection("event_contexts").add(
                {
    ...
    def get_event_contexts(self, user_id: int) -> dict[str, str]:
        ...
        if self.use_firestore:
            docs = self.db.collection("users").document(str(user_id)).collection("event_contexts").stream()
            ...
            with sqlite3.connect(self._sqlite.db_path) as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT event_title, context_description FROM event_contexts WHERE user_id = ?", (user_id,)
                )
    ...
    def save_conversation(self, user_id: int, messages: list[dict[str, str]]):
        ...
        if self.use_firestore:
            self.db.collection("users").document(str(user_id)).update(
                {"conversation_history": messages, "conversation_updated": firestore.SERVER_TIMESTAMP}
            )
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        result = {"backend": "firestore" if self.use_firestore else "sqlite", "connected": False, "error": None}
