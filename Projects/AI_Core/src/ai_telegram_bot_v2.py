        ...
        try:
            ...
            if user_id != ADMIN_ID:
                await context.bot.send_message(
                    chat_id=ADMIN_ID,
                    text=(
                        f"🐝 User {user_name} contributed a Gemini API key to the {branch_id} swarm!"
                    ),
                )
        except Exception as e:
            await update.message.reply_text(f"❌ Error: {e}")
...
try:
    ...
    logger.warning(
        f"Failed to parse ALLOWED_USERS '{ALLOWED_USERS_STR}': {e}. Falling back to Igor-only."
    )
    ALLOWED_IDS = [IGOR_ID]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ai_response or not ai_response.strip():
        ai_response = (
            "🤔 Не удалось получить ответ. Попробуйте ещё раз или смените провайдер."
        )
    ...
    for title, content in notion_matches:
        if notion_client:
            try:
                ...
                if url:
                    ai_response = ai_response.replace(
                        tag_re, f"📝 _(Заметка Notion создана: [{title}]({url}))_"
                    )
                else:
...
async def query_ollama_with_context(
    ...
    logger.info(
        f"[AI] Querying {user_id} (Branch: {branch_id}, Context: {project_context}), prompt: {len(prompt)}"
    )
...
async def dream_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if gh_handler:
        ...
        try:
            ...
            await msg.edit_text(
                "⚠️ Мечта запомнена локально, но GitHub временно недоступен. Вернёмся к ней позже!"
            )
    else:
