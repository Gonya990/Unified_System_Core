    ...
    if data == "connect_google":
        ...
        if auth_url:
            await query.edit_message_text(
                ...
                f"1. ⚠️ <b>СОВЕТ:</b> Если ссылка не открывается, <b>скопируйте её</b> и откройте в обычном браузере (Chrome/Safari).\n\n"
                f"2. Ссылка для входа:\n{auth_url}\n\n"
                f"3. Авторизуйте бота в своем аккаунте Гугл.\n\n"
                f"4. 🚨 <b>ВАЖНО</b>: После авторизации вы увидите ошибку <i>«Не удается открыть страницу»</i> (или белый экран). <b>ЭТО НОРМАЛЬНО!</b>\n\n"
                f"5. <b>Что делать:</b>\n"
                f"   • Нажмите на адресную строку браузера\n"
                f"   • Скопируйте ВСЮ ссылку (она длинная)\n"
                f"   • Пришлите её сюда боту.\n\n"
                f"Бот сам найдет нужный код. 🦾",
                parse_mode='HTML'
            )
        else:
