...
class GitHubHandler:
    ...
    def __init__(self, token: str, repo: str = "Unified-system-Core/Unified_System_Core"):
        self.token = token
        self.repo = repo
    ...
    async def create_issue(self, title: str, body: str, labels: list = None) -> bool:
        ...
        payload = {"title": title, "body": body, "labels": labels or ["bug", "self-healing"]}
            ...
            async with aiohttp.ClientSession(headers=self.headers) as session:
                async with session.post(url, json=payload) as resp:
                    if resp.status == 201:
                        ...
                        logger.error(f"Failed to create GitHub Issue: {resp.status} - {text}")
                        return False
