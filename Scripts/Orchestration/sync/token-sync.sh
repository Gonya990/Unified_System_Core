...
for NODE in "${NODES[@]}"; do
    ...
    if [ "$NODE" = "igor-gaming" ]; then
        ...
        ssh -o ConnectTimeout=5 "$NODE" "wsl bash -c \"mkdir -p /home/gonya/.config/unified-system\""
        # Sync via wsl tee to write directly to WSL filesystem
        if ssh -o ConnectTimeout=5 "$NODE" "wsl bash -c \"tee /home/gonya/.config/unified-system/tokens.yaml > /dev/null\"" < "$VAULT_PATH"; then
