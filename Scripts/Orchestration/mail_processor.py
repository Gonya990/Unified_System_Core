...
class ProcessorConfig:
    ...
    alert_keywords: List[str] = field(default_factory=lambda: [
        'urgent', 'critical', 'emergency', 'alert', 'error', 'failure',
        'security', 'breach', 'down', 'offline', 'blocked', 'help',
        'immediate', 'asap', 'priority', 'warning', 'incident'
    ])
    
    # Council Agents - their messages get special processing
    council_agents: List[str] = field(default_factory=lambda: [
        'FuchsiaCat', 'VioletCastle', 'PinkLake',
        'Kostya', 'WhiteMill', 'IvoryOtter', 'CalmSnow'
    ])
    ...
    # Persistence
    processed_ids_file: str = field(default_factory=lambda: 
        os.path.join(os.path.dirname(__file__), '.mail_processor_state.json'))
...
"""
    ...
    def _process_council_message(self, message: Dict[str, Any]):
        ...
        if message.get('ack_required'):
            try:
                ...
                self.logger.info(f"Auto-acknowledged message #{message.get('id')}")
            except Exception as e:
    ...
    def process_message(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Processing message #{msg_id} from {sender}: {subject}")
            ...
            try:
                ...
                self.logger.warning(f"Could not mark message #{msg_id} as read: {e}")
