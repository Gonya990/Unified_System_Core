...
def call_mcp(method, params):
    payload = {"jsonrpc": "2.0", "method": method, "id": int(time.time()), "params": params}
    try:
...
def request_telegram_approval(sender, subject, body):
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            payload = {"sender": sender, "subject": subject, "body": body, "summary": summary}
            cursor.execute(
                """
...
def main():
    ...
    while True:
        try:
            ...
            resp = call_mcp(
                "tools/call", {"name": "fetch_inbox", "arguments": {"project_key": PROJECT_KEY, "agent_name": MY_AGENT}}
            )
                ...
                if sender in TARGET_AGENTS and not is_read:
                    ...
                    call_mcp(
                        "tools/call",
                        {
                            ...
                            "arguments": {"project_key": PROJECT_KEY, "agent_name": MY_AGENT, "message_id": msg_id},
                        },
                    )
