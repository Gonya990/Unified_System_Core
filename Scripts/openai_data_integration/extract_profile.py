...
def extract_profile(data_dir: Path) -> dict:
    ...
    profile = {
        ...
        "user_info": {},
        "preferences": {},
        "custom_instructions": {"about_user": "", "response_style": ""},
        "data_controls": {},
        "subscription": {},
    }
    ...
    for filename in ["user.json", "account.json", "profile.json"]:
        ...
        if filepath.exists():
            print_bilingual(f"Found {filename}, extracting...", f"Найден {filename}, извлечение...", Colors.YELLOW)
...
def create_agent_preferences(profile: dict, output_path: Path):
    ...
    agent_prefs = {
        ...
        "openai_sync": {"last_sync": datetime.now().isoformat(), "auto_sync": False, "sync_interval_days": 7},
    }
...
def main():
    ...
    # Extract profile
    print_bilingual("Extracting profile information...", "Извлечение информации профиля...", Colors.YELLOW)
    ...
    print_bilingual("Creating agent preferences...", "Создание настроек агента...", Colors.YELLOW)
    ...
    print_bilingual("Creating profile documentation...", "Создание документации профиля...", Colors.YELLOW)
    ...
    print_bilingual("✓ Profile extraction complete!", "✓ Извлечение профиля завершено!", Colors.GREEN)
