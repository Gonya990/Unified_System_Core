...
SECRET_ID_PATTERN = (
    r"projects/(?P<project>\d+)/secrets/(?P<secret>[\w\-]+)/versions/(?P<version>\d+)"
)

def main():
    parser = argparse.ArgumentParser(
        description="Audit GCP Secret Manager for drift and rotation compliance."
    )
    ...
    for expected in EXPECTED_SECRETS:
        if expected not in found_secrets:
            ...
            violations: List[str] = []
            replication = (
                secret_obj.replication.automatic or secret_obj.replication.user_managed
            )
            if (
                AUDIT_POLICIES.get("replication_automatic")
                and "automatic" not in str(replication)
            ):
                violations.append(
                    f"Secret {secret_obj.name} does not have automatic replication."
                )
            
            if violations:
                logger.warning(
                    f"⚠️ Policy violations for secret {expected}: "
                    f"{', '.join(violations)}"
                )
            ...
            try:
                ...
                versions = client.list_secret_versions(
                    request={"parent": secret_obj.name}
                )
                latest_version = None
                for version in versions:
                    if version.state == secretmanager.SecretVersion.State.ENABLED:
                        ...
                        if (
                            ...
                            or version.create_time > latest_version.create_time
                        ):
                ...
                if latest_version:
                    ...
                    stale_secrets.append({
                        "name": expected, 
                        "age_days": -1, 
                        "latest_version": "None (No enabled versions)"
                    })
    ...
    if stale_secrets:
        ...
        for s in stale_secrets:
            print(
                f"   - {s['name']}: {s['age_days']} days old "
                f"(Version: {s['latest_version']})"
            )
