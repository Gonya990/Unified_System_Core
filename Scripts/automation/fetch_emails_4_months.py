...
def main():
    ...
    # Pagination loop
    while True:
        results = service.users().messages().list(userId='me', q=query, pageToken=next_page_token, maxResults=500).execute()
    ...
    for i, msg in enumerate(messages):
        try:
            ...
            keywords = ['job', 'vacancy', 'hiring', 'opportunity', 'career', 'resume', 'apply', 
                        'משרה', 'דרוש', 'גיוס', 'קורות חיים', 'עבודה', 
                        'linkedin', 'alljobs', 'glassdoor', 'indeed', 
                        'manager', 'operation', 'supervisor', 'technical', 'site manager']
