...
def create_pdf(jobs):
    ...
    # Custom Styles
    styles.add(
        ParagraphStyle(name="JobTitle", parent=styles["Heading2"], fontSize=12, spaceAfter=6, textColor=colors.darkblue)
    )
    ...
    story.append(
        Paragraph("These are direct contacts for recruitment agencies and companies in your field.", styles["Normal"])
    )
    ...
    for job in jobs:
        ...
        if email_contact:
            ...
            story.append(
                Paragraph(f"<b>Contact Email:</b> <a href='mailto:{email_contact}'>{email_contact}</a>", styles["Link"])
            )
        elif link_contact:
