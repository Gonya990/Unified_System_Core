...
- [x] **Авто-обновление:** Механизм обновления через `/update`
      (Git Pull + Pip + Restart).
...
- [x] **HA Расширение:** Сенсоры, скрипты, сцены
      (`/ha sensors`, `/ha script`, `/ha scene`).
...
- [x] **Linear API:** Интеграция с Linear.app для таск-менеджмента
      (`/linear`).
- [x] **Daily Digest:** Ежедневная сводка (статистика, задачи, мотивация)
      каждое утро в 09:00.
- [x] **Google Calendar:** Интеграция календаря (события в digest, `/calendar`).
- [x] **HomeKit Bridge:** Мост для Apple Home
      (автоматическое добавление устройств из HA).
...
- [x] **SmartThings Guide:** Полное руководство по исправлению
...
- [x] **Проверка:** Протестировать все новые функции
      (HomeKit, Calendar, Linear, Digest).
