...
ssh -o ConnectTimeout=10 root@100.74.137.122 "hostname && uptime && pvesm status && qm list"
```

### 2. Storage Health
```bash
ssh root@100.74.137.122 "pvesm status && echo '---' && df -h && echo '---' && lvs --units g"
```

### 3. Critical VMs Status
```bash
# VM 106 (unified-home-core-cloud) - main AI workloads
ssh root@100.74.137.122 "qm status 106 && qm config 106 | grep -E 'scsi|memory|cores|name'"

# VM 100 (Gaming) - if relevant
ssh root@100.74.137.122 "qm status 100"
```

### 4. Network/Tailscale Check
```bash
# Check if key nodes are reachable via Tailscale
ping -c 1 -W 2 100.74.137.122  # proxmox-gpu
