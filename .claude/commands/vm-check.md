...
```bash
ssh root@100.74.137.122 "qm status <VMID> && echo '---' && qm config <VMID>"
```

### 2. Check Disk Allocation
```bash
# Parse scsi/virtio disks from config, check their storage pools
ssh root@100.74.137.122 "qm config <VMID> | grep -E 'scsi|virtio|ide|sata'"
```
...
ssh root@100.74.137.122 "lvs <pool> --units g"  # if LVM-based
```

### 4. Check Inside VM (if running)
```bash
# Get VM's IP from config or ARP table
ssh root@100.74.137.122 "ip neigh | grep <MAC_FROM_CONFIG>"

# SSH into VM and check disk usage
ssh root@100.74.137.122 "ssh user@<VM_IP> 'lsblk && echo --- && df -h'"
```
...
Host: proxmox-gpu (100.74.137.122)
