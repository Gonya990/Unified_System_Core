# Task: YouTube Token Update & Mashov Integration

## Phase 1: YouTube Brand Account Migration <!-- id: 100 -->

- [x] Obtain authorization code for new brand account <!-- id: 101 -->
- [x] Exchange code for new YouTube API token <!-- id: 102 -->
- [x] Save new token and verify access <!-- id: 103 -->
- [x] Update Content Factory to use new token <!-- id: 104 -->

## Phase 2: Mashov Integration & Debugging <!-- id: 200 -->

- [x] Authenticate with Mashov using cookie session <!-- id: 201 -->
- [x] Resolve School ID ambiguity (Psagot Carmiel 244285) <!-- id: 202 -->
- [/] Resume homework and grade extraction (via Webtop) <!-- id: 203 -->
  <!-- CURRENT FOCUS -->

## Phase 3: System Maintenance <!-- id: 300 -->

- [x] Sync Agent Mail inbox <!-- id: 301 -->
- [x] Sync Task Board (Beads) <!-- id: 302 -->
- [ ] Perform Vibranium Full Sync <!-- id: 303 -->
- [ ] Autosave and push changes <!-- id: 304 -->
